...
int main(){
    ...
    int output_value;
    int remainder = 0; //the amount of length we will have remaining after the cutting is finished
    ...
    while(scanf("%d %d", &rod_list_length, &rod_list_value) != EOF){ //IF the user types a 'ctrl-z' the output fo scanf will be EOF
        if(rod_list_length <= supplied_rod_length && rod_list_length > 0){ //If the user gives us a rod BIGGER than the supplied rod, we can never use this value and throw it out
